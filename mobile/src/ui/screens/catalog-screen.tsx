...
export function CatalogScreen({ initialFilter }: CatalogScreenProps = {}) {
  ...
  const toggle = useCallback((id: number) => setSelected((current) => { const next = new Set(current); next.has(id) ? next.delete(id) : next.add(id); return next; }), []);
  const open = useCallback((id: number) => router.push({ pathname: "/items/[id]", params: { id: String(id) } }), []);
  ...
  const sellSelected = async () => { const ids = [...selected]; if (!(await confirm({ title: "Marcar como vendidas", message: `Se marcarán ${ids.length} ${ids.length === 1 ? "prenda" : "prendas"} como vendidas en una sola operación. Si alguna no está disponible, no se modificará ninguna.`, confirmLabel: "Confirmar venta", tone: "danger", icon: "hand-coin-outline" }))) return; try { await inventoryService.sellItems(ids); setSelected(new Set()); await loadFirst(); notify({ message: `${ids.length} ${ids.length === 1 ? "prenda vendida" : "prendas vendidas"}.`, tone: "success", undo: async () => { await inventoryService.restoreSales(ids, "Venta deshecha"); await loadFirst(); } }); } catch (reason) { notify({ message: reason instanceof Error ? reason.message : "No se modificó ninguna prenda.", tone: "error" }); } };
  ...
  const empty = !loading ? <ScreenState title="No encontramos prendas" body="Cambie los filtros o agregue una nueva prenda." action={<AppButton label="Agregar prenda" icon="plus" onPress={() => router.push("/items/new")} />} /> : null;

  return <View style={styles.root}>
    <View style={styles.toolbar}><Searchbar accessibilityLabel="Buscar prendas" value={search} onChangeText={setSearch} placeholder="Código, precio, descripción o ubicación" style={styles.search} /><View style={styles.viewSwitch}>{(["grid", "list"] as const).map((value) => <Pressable key={value} accessibilityRole="button" accessibilityLabel={value === "grid" ? "Vista cuadrícula" : "Vista lista"} accessibilityState={{ selected: view === value }} onPress={() => setView(value)} style={[styles.viewButton, view === value && styles.viewActive]}><MaterialCommunityIcons name={value === "grid" ? "view-grid-outline" : "view-list-outline"} size={22} color={view === value ? colors.onPrimary : colors.primary} /></Pressable>)}</View><Pressable accessibilityRole="button" accessibilityLabel="Vista rápida a pantalla completa" onPress={openQuickView} style={({ pressed }) => [styles.quickButton, pressed && styles.pressed]}><MaterialCommunityIcons name="cellphone" size={20} color={colors.onPrimary} /></Pressable></View>
