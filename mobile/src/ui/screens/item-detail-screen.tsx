...
import { ActivityIndicator, FlatList, KeyboardAvoidingView, Modal, Pressable, ScrollView, StyleSheet, Text, TextInput, useWindowDimensions, View } from "react-native";
...
export function ItemDetailScreen() {
  ...
  const restore = async () => {
    ...
    try { await inventoryService.restoreSale(item.id, "Restauración confirmada"); await refresh(); } catch (reason) { void alert({ title: "No pudimos restaurar la venta", message: reason instanceof Error ? reason.message : "Intente nuevamente.", tone: "danger" }); }
  };
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
    ...
    <View style={styles.hero}><View style={styles.heroTop}><View><Text style={styles.eyebrow}>{item.machineCode}</Text><Text accessibilityRole="header" style={styles.title}>{item.code}</Text></View><StatusChip status={item.status} /></View><Text style={styles.price}>{formatMoney(item.status === "sold" ? item.soldPrice ?? item.price : item.price)}</Text><Text style={styles.location}>{item.currentLocation?.name ?? "Sin asignar / En transición"}</Text><Text style={styles.description}>{item.description || "Sin descripción"}</Text>{item.tags.length ? <View style={styles.tags}>{item.tags.map((tag) => <View key={tag} style={styles.tag}><Text style={styles.tagText}>{tag}</Text></View>)}</View> : null}</View>
    <View style={styles.actions}>
      {item.status === "active" ? <AppButton grow tone="danger" label="Vender" icon="hand-coin-outline" onPress={() => setSaleOpen(true)} /> : <AppButton grow tone="secondary" label="Restaurar" icon="backup-restore" onPress={() => void restore()} />}
      <AppButton grow label="Mover" icon="swap-horizontal" onPress={() => router.push({ pathname: "/transfer", params: { ids: String(item.id) } })} disabled={item.status !== "active"} />
    </View>
    <View style={styles.actions}>
...
function SalePanel({ visible, item, onClose, onSaved }: { visible: boolean; item: Item; onClose: () => void; onSaved: () => void }) { const { alert } = useDialog(); const [price, setPrice] = useState(""); const [date, setDate] = useState(new Date().toISOString().slice(0,10)); const [saving, setSaving] = useState(false); const save = async () => { setSaving(true); try { await inventoryService.sellItem(item.id, { soldPrice: price || undefined, soldAt: date }); onSaved(); } catch (error) { void alert({ title: "No pudimos registrar la venta", message: error instanceof Error ? error.message : "Revise la información.", tone: "danger" }); } finally { setSaving(false); } }; return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}><KeyboardAvoidingView behavior={process.env.EXPO_OS === "ios" ? "padding" : undefined} style={styles.modalBackdrop}><View style={styles.modal}><Text style={styles.modalTitle}>Marcar como vendida</Text><Text style={styles.muted}>El precio real es opcional. La última ubicación y las fotos se conservarán.</Text><TextInput value={price} onChangeText={setPrice} keyboardType="decimal-pad" placeholder={`Precio real (referencia ${item.price})`} style={styles.input} /><TextInput value={date} onChangeText={setDate} placeholder="AAAA-MM-DD" style={styles.input} /><View style={styles.actions}><AppButton grow tone="quiet" label="Cancelar" icon="close" onPress={onClose} /><AppButton grow tone="danger" label={saving ? "Guardando..." : "Confirmar venta"} icon="hand-coin-outline" onPress={() => void save()} disabled={saving} /></View></View></KeyboardAvoidingView></Modal>; }
