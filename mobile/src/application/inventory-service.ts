...
export class InventoryService {
  ...
  sellItem(itemId: number, input: { soldPrice?: string; soldAt?: string } = {}) { return this.sales.sell(itemId, input); }
  sellItems(itemIds: number[], input: { soldPrice?: string; soldAt?: string } = {}) { return this.sales.sellMany(itemIds, input); }
  restoreSale(itemId: number, reason: string) { return this.sales.restore(itemId, reason); }
  restoreSales(itemIds: number[], reason: string) { return this.sales.restoreMany(itemIds, reason); }
  ...
  updateSetting<K extends keyof Omit<AppSettings, "lastBackupAt" | "backupDue" | "backupDueInDays">>(key: K, value: AppSettings[K]) { return this.settings.set(key, value); }
